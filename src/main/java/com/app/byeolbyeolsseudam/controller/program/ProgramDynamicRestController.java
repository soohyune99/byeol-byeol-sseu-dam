...
public class ProgramDynamicRestController {
    ...
    *  다른 문제인지 확인 하기 위해 처음 들어왔을때 확인하는 페이지 경로 만들었었음.
    *  확인 OK
    *  */
...
//    }
    /*
    * 처음에는 경로를 하나로만 작성하면 알아서 경로가 생기는 걸로 인지 했었음
    * 하지만 그게 아니었음. 여러가지 경우의 수 대로 나누어주었어야 했고
    * "/dynamic/{page}/{keyword}"
    * "/dynamic/{page}/{programStatus}"
    * => 이 두개는 동일 경로로 인식. 이것을 구분해줄 작업이 필요했음
    * "/dynamic/{page}/status/{programStatus}" 가운데에 status라는 것을 넣어주어 키워드만 입력되었을때, 상태 선택이 되었을 때 2가지로 분리 되는 것을 할 수 있었음.
    */
